...
import { Send, Users, AlertCircle, MessageSquare, Plus, X, Edit2, Save, Link, UserPlus, UserMinus, Shield, LogOut, Copy, Check, Paperclip, Download, Image, FileText, Video, File } from 'lucide-react';
...
const GroupChatDashboard = () => {
  ...
  // File upload API call function
  const fileApiFetch = async (path, formData) => {
    try {
      const response = await fetch(`${BASE_URL}/api/v1${path}`, {
        method: 'POST',
        credentials: 'include',
        ...
        if (data.messages) {
          ...
          for (let i = 0; i < data.messages.length; i++) {
            ...
            formattedMessages.push({
              id: msg.id,
              text: msg.userMessage || msg.content, // Use userMessage for display
  ...
  const sendFile = async () => {
    ...
    try {
      ...
      formData.append('groupId', currentGroupId.toString());
      
      const data = await fileApiFetch('/user-message/file', formData);
  ...
  // FIXED: View file function - proper URL construction and modal handling
  const viewFile = async (fileInfo, messageType) => {
    if (!fileInfo) {
      setError('File information not found');
      return;
    }
    ...
    try {
      ...
      if (fileInfo.storedFilename) {
        filename = fileInfo.storedFilename;
      } else if (fileInfo.filePath) {
        const pathParts = fileInfo.filePath.split('/');
        filename = pathParts[pathParts.length - 1];
      } else if (fileInfo.fileName && fileInfo.id) {
        // Construct expected filename format if we have original name and ID
        const extension = fileInfo.fileName.includes('.') 
          ? fileInfo.fileName.substring(fileInfo.fileName.lastIndexOf('.'))
          : '';
        filename = `${fileInfo.id}-${fileInfo.fileName}${extension}`;
      } else {
        setError('Could not determine file location');
        setFileViewLoading(false);
        return;
      }
      
      // Use the correct API endpoint from routes
      const fileUrl = `${BASE_URL}/api/v1/file/${filename}`;
      
      console.log('Attempting to view file:', { fileInfo, filename, fileUrl });
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          setError('Access denied to file');
        } else {
          setError(`Error accessing file: ${response.status}`);
        }
        setFileViewLoading(false);
        return;
      }
      
      // File is accessible, set up for viewing
      setViewingFile({
        fileName: fileInfo.fileName || `file.${messageType === 'image' ? 'jpg' : messageType === 'video' ? 'mp4' : 'txt'}`,
        url: fileUrl,
        type: messageType,
  ...
  // FIXED: Download file function - proper URL construction and download handling
  const downloadFile = async (fileInfo) => {
    ...
    try {
      ...
      // Use the correct download endpoint from routes
      const downloadUrl = `${BASE_URL}/api/v1/download/${filename}`;
      
      console.log('Attempting to download file:', { fileInfo, filename, downloadUrl });
      
      // Create temporary anchor element for download
      const link = document.createElement('a');
      link.href = downloadUrl;
      link.download = fileInfo.fileName || 'download'; // Use original filename for download
      ...
      <div className="chat-container">
        <div className="sidebar">
          ...
          <div className="sidebar-content">
                  ...
                  <div className="file-preview-content">
                    <div className="file-preview-info">
                      <div className="file-icon-large">
                        {selectedFile.type.startsWith('image/') && <Image size={48} />}
                        {selectedFile.type.startsWith('video/') && <Video size={48} />}
                        {!selectedFile.type.startsWith('image/') && !selectedFile.type.startsWith('video/') && <FileText size={48} />}
                      </div>
                    ...
                    <div className="form-actions">
                      ...
                      <button 
                        ...
                        className="submit-button"
                      >
                        {fileUploadLoading ? 'Uploading...' : 'Send File'}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            {/* IMPROVED File Viewer Modal */}
            {showFileViewer && viewingFile && (
              <div className="create-group-modal file-viewer-overlay">
                <div className="modal-content file-viewer-modal">
                  <div className="modal-header">
                    <h4>
                      {viewingFile.type === 'image' && <Image size={20} />}
                      {viewingFile.type === 'video' && <Video size={20} />}
                      {viewingFile.type !== 'image' && viewingFile.type !== 'video' && <FileText size={20} />}
                        ...
                        {viewingFile.type === 'image' ? (
                          ...
                          <div className="file-info-viewer">
                            ...
                            <div className="file-viewer-details">
                              ...
                              {viewingFile.fileSizeFormatted && (
                                <p>Size: {viewingFile.fileSizeFormatted}</p>
                              )}
                              <p>File preview not available for this type</p>
                              <p>Click download to save and view the file</p>
                            </div>
                          </div>
                        )}
                      </>
        ...
        <div className="chat-main">
                          ...
                          {isFileMessage ? (
                            <div className="file-message">
                              <div className="file-info">
                                ...
                                <div className="file-details">
                                  <p className="file-name">{message.text}</p>
                                  {message.fileInfo && message.fileInfo.fileSizeFormatted && (
                                    <p className="file-size">{message.fileInfo.fileSizeFormatted}</p>
                                  )}
                                </div>
                              </div>
                              {message.fileInfo && (
                                <div className="file-actions">
                                  {(message.messageType === 'image' || message.messageType === 'video') && (
                                    <button 
                                      onClick={() => viewFile(message.fileInfo, message.messageType)}
                                      className="file-action-btn view-btn"
                                      disabled={fileViewLoading}
                                      title={`View ${message.messageType}`}
                                    >
                                      {message.messageType === 'image' ? <Image size={14} /> : <Video size={14} />}
                                      View
                                    </button>
                                  )}
                                  <button 
                                    onClick={() => downloadFile(message.fileInfo)}
          ...
          <div className="message-input-area">
            ...
            <button 
              ...
              disabled={loading || fileUploadLoading || !currentGroupId || groups.length === 0}
